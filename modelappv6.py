...
shipping_cost_rate = st.number_input("Shipping Cost Rate (per mile per order unit, in $)", min_value=0.0, value=1.0, step=0.1, format="%.2f")
unit_cost = st.number_input("Unit Cost (per unit, in $)", min_value=0, value=10, step=1, format="%.0f")
...
# =============================================================================
# במקרה של Main Regionals, עבור כל MAIN warehouse שמשרת יותר מאזור,
# נבקש מהמשתמש להזין מידע על המרחק הממוצע (במיילים) עבור כל אזור נוסף.
# הקלט יהיה כמחרוזת בפורמט "market1:distance1, market2:distance2, ..."
...
if layout_type == "Main Regionals":
    ...
    # Also, input shipping cost per order unit per mile for additional markets:
    shipping_cost_per_order_unit = st.number_input("Shipping Cost per Order Unit per Mile (for additional markets, in $)", min_value=0.0, value=0.5, step=0.1, format="%.2f")

# =============================================================================
# Rental Parameters (NEW)
# =============================================================================
st.markdown("<p class='subheader-font'>Rental Parameters</p>", unsafe_allow_html=True)
sq_ft_per_unit = st.number_input("Square feet required per unit (default 0.8)", min_value=0.0, value=0.8, step=0.1, format="%.1f")
overhead_factor_main = st.number_input("Overhead factor for MAIN warehouse (default 1.2)", min_value=1.0, value=1.2, step=0.1, format="%.1f")
overhead_factor_front = st.number_input("Overhead factor for FRONT warehouse (default 1.5)", min_value=1.0, value=1.5, step=0.1, format="%.1f")
...
# =============================================================================
# Helper Functions for Rental Calculation (Existing)
# =============================================================================
...
# =============================================================================
# Rental Cost Calculation (Existing)
# =============================================================================
...
# =============================================================================
# Inventory Financing Calculation (Existing)
# =============================================================================
...
# New input: Container capacity for 40ft HC (units)
container_capacity_40 = st.number_input("Container Capacity for 40ft HC (units, default 600)", min_value=0, value=600, step=1, format="%d")

if st.button("Calculate Shipping Costs"):
    total_sea_shipping_cost = 0.0
    total_land_shipping_cost = 0.0

    # --- Sea Shipping Cost Calculation (applies to both scenarios) ---
    # Assume one MAIN warehouse serves all market areas (for Central & Fronts)
    if layout_type in ["Central and Fronts", "Main Regionals"]:
        # For each area served by the MAIN warehouse (or each MAIN warehouse in Main Regionals)
        if layout_type == "Central and Fronts":
            main_wh = next((wh for wh in warehouse_data if wh["type"] == "MAIN"), None)
            if main_wh is None:
            ...
            # For each MAIN warehouse, sum over its served markets
            for wh in warehouse_data:
                if wh["type"] == "MAIN":
                    wh_sea_cost = 0.0
                    for area in wh["served_markets"]:
    ...
    # --- Land Shipping Cost Calculation ---
    if layout_type == "Central and Fronts":
        # For each FRONT warehouse:
        for wh in warehouse_data:
            if wh["type"] == "FRONT":
                warehouse_land_cost = 0.0
                # For each month, compute monthly forecast for this FRONT warehouse
                for m in range(12):
                    monthly_forecast = 0.0
                    for area in wh["served_markets"]:
                        if area in market_area_data:
                            monthly_forecast += market_area_data[area]["forecast_demand"][m]
                    weekly_demand = monthly_forecast / 4.0  # average weekly demand
                    # Compute cost per unit ("panel") for shipping from MAIN to this FRONT warehouse:
                    ...
                    warehouse_land_cost += weekly_shipping_cost * 4  # convert weekly cost to monthly
                total_land_shipping_cost += warehouse_land_cost
    elif layout_type == "Main Regionals":
        # For each MAIN warehouse that serves more than one market:
        for wh in warehouse_data:
            if wh["type"] == "MAIN":
                # Determine primary market: assume the first market in its served_markets list is primary.
                served = wh["served_markets"]
                if served:
                    primary_market = served[0]
                    wh_land_cost = 0.0
                    # For each additional market (if any)
                    for area in served[1:]:
                        if area in market_area_data:
                            annual_forecast = sum(market_area_data[area]["forecast_demand"])
                            avg_order = market_area_data[area]["avg_order_size"]
                            # Number of orders (or units) = annual_forecast / avg_order
                            orders = annual_forecast / avg_order if avg_order > 0 else 0
                            # Get distance for this market from additional_market_distances (if provided)
                            distance = additional_market_distances.get(area, 0)
                            # Land shipping cost for this area:
